...
class ParabolicJointSegment:
    ...
    def f_prime(self, s):
        return 2 * self.a * s + self.b
    
    def f_prime2(self, s):
        return 2 * self.a
    
class JointPath:
    def __init__(self, joint_positions, vel_constraint, accel_constraint, parabolic=False):
        ...
        if parabolic:
            self.segments = ParabolicJointSegment(-720, 720, 0)
        else:
    ...
    def f_prime(self, s):
        if self.parabolic:
            return self.segments.f_prime(s)
        else:
            if s == 0:
    ...
    def f_prime2(self, s):
        if self.parabolic:
            return self.segments.f_prime2(s)
        else:
            segment_idx = math.floor(s/self.s_interval)
